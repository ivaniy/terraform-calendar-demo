...
module "first_deploy_ami" {
  source = "./modules/ami"
  master_ami = "${aws_ami_from_instance.master_image.id}"
  private_subnet_ids = "${module.dolyuk_vpc.private_subnets_ids}"
  calendar_key = "${var.calendar_key}"
  calendar_key_name = "${aws_key_pair.calendar_key.key_name}"
  security_groups = ["${module.dolyuk_vpc.security_group_websg_id}"]
  bastion_ip = "${module.dolyuk_vpc.bastion_ip}"
  bastion_user = "${module.dolyuk_vpc.bastion_user}"
  bastion_private_key = "${var.nat_gw_key}"
  db_host_name="${module.db_instance.db_address}"
...
module "autoscaling" {
  ...
  aws_ami_id = "${module.first_deploy_ami.base_ami}"
